...
```{r}
#### LASSO
## Let's define the predictor matrix X.
X_Lasso <- model.matrix(fast_growing ~ ., data = Data_2012)[,-1]

### Create interaction terms (except for flags)

# Identify variables containing the word "flag"
names_to_exclude <- grep("flag", colnames(X_Lasso), value = TRUE)

# Filter variables that do not contain the word "names"
X_without_flag <- X_Lasso[, !colnames(X_Lasso) %in% names_to_exclude]

# Generate interaction terms for the remaining variables
X_Lasso_interaction <- model.matrix(~.^2, data.frame(X_without_flag))

# Combine them
X_Lasso <- cbind(X_Lasso, X_Lasso_interaction)

## Let's define the response vector Y.
Y_Lasso <- Data_2012$fast_growing
...
set.seed(20231210)
# Fit the model
lasso_fit <- cv.glmnet(X_Lasso, Y_Lasso, family = "binomial", alpha = 1, nfolds = 5)

# Get the optimal value of lambda
lambda_optimal <- lasso_fit$lambda.min

```

```{r}
## Report the optimal lambda.
cat("Optimal lambda:", lambda_optimal_interactions, "\n")

## get the RMSE (in 5 test set) from lasso_fit corresponding the the optimal value of lambda.
cvm_optimal <- lasso_fit$cvm[lasso_fit$lambda == lambda_optimal] # This is MSE.

RMSE_CV_Lasso <- sqrt(cvm_optimal)
...
Lasso_optimal <- glmnet(X_Lasso, Y_Lasso, family = "binomial", alpha = 1, lambda = lambda_optimal)
```
