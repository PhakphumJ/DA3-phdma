...
Data_panel <- Data_panel %>%
  mutate(gender_m = factor(gender, levels = c("female", "male", "mix")),
         region_m = factor(region_m, levels = c("Central", "East", "West")), urban_m = factor(urban_m, levels = c(1,2,3)))
...
Data_panel <- Data_panel %>%
  mutate(fixed_assets_ratio = fixed_assets / total_assets,
         fixed_assets_growth_rate = (fixed_assets/lag(fixed_assets) - 1)*100)
```

Similary, intangible assets is important in the same way as it captures the assets acquired mainly for the purpose of long-term growth.

```{r}
#### Create Intangible Assets Ratio
Data_panel <- Data_panel %>%
  mutate(intang_assets_ratio = intang_assets / total_assets,
         intang_assets_growth_rate = (intang_assets/lag(intang_assets) - 1)*100)
```

Personnel expenditure is also another important variable. 
