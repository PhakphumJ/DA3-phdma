...
```{r}
### Compile the results of expected loss into a table.

# Create dataframe of expected loss.
expected_loss <- data.frame(model = c("Logit", "LASSO", "Random Forest"),
                            expected_loss = c(expected_loss_logit, expected_loss_lasso, expected_loss_RF))

# Print expected loss.
kable(expected_loss, digits = 2, align = "c") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "bordered"))
```
...
I pick the logistic regression model 2 as to carry out classisification in both datasets as it is the model that minimize the expected loss in Task 1. 
