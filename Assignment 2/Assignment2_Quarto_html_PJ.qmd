...
One part of the task is to compare the results with the case study in London. Therefore, to make the results comparable I convert the price from THB to GBP using the exchange rate on the scraping date which is 0.0238 at 29-03-2023 and 0.0225 at 22-09-2023[^1].

[^1]: The exchange rate is from https://finance.yahoo.com/quote/THBGBP%3DX/history?p=THBGBP%3DX 
...
hold_out_Mar$price <- hold_out_Mar$price * 0.0238

Data_Mar$price <- Data_Mar$price * 0.0238
```
...
for (parameter in 1:length(minsplit_range)) {
  results <- list()
   for (i in 1:5) {
    ...
    # Estimate the model
    model_bagging <- bagging(price ~ ., data = train, nbagg = 25, control = rpart.control(minsplit = minsplit_range[parameter]))
...
```{r}
## Use OLS model 3 to predict the price and calculate the RMSE
hold_out_Mar$price_pred <- predict(model_OLS_3, newdata = hold_out_Mar)

# Calculate the RMSE
RMSE_OLS_Holdout <- RMSE(hold_out_Mar$price, hold_out_Mar$price_pred)
...
## Use the random forest model to predict the price and calculate the RMSE

hold_out_Mar$price_pred <- predict(model_RF, newdata = hold_out_Mar)

# Calculate the RMSE
RMSE_RF_Holdout <- RMSE(hold_out_Mar$price, hold_out_Mar$price_pred)
