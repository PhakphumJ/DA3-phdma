...
3. Model 3: Model 2 + squared terms of variables in model 1
...
Model 3: *price* = model 2 + $accommodates^2$, $bedrooms^2$, $beds^2$
...
Model 5: *price* = model 5 + *near_MRT*, *near_BTS*, *near_ARL*
...
for (i in 1:5) {
  ...
  # Estimate the model
  model_OLS_3 <- update(model_OLS_2, . ~ . +  I(accommodates^2) + I(bedrooms^2) + I(beds^2), data = train)
...
```{r}
## Use OLS model 1 to predict the price and calculate the RMSE in the holdout set.
