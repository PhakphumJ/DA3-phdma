...
## property_type
Data_Mar = dummy_cols(Data_Mar, select_columns = "property_type", remove_first_dummy = TRUE)
...
# Deleting property_type column
hold_out_Mar <- hold_out_Mar %>%
  select(-property_type)

hold_out_Sept <- hold_out_Sept %>% 
  select(-property_type)
...
RMSE_OLS_1 <- mean(unlist(results))
...
for (i in 1:5) {
  ...
  # Estimate the model
  model_OLS_2 <- update(model_OLS_1, . ~ . + accommodates + bedrooms + beds + num_baths + num_shared_baths + is_half_bath + num_private_baths + Hair_dryer_dummy + Shampoo_dummy + TV_dummy + Wifi_dummy + Shower_gel_dummy + Air_conditioning_dummy + Kitchen_dummy + Elevator_dummy + Free_parking_on_premises_dummy + Gym_dummy + Pool_dummy + Washer_dummy + Dryer_dummy + Essentials_dummy + Iron_dummy + Smoking_allowed_dummy + Luggage_dropoff_allowed_dummy + Refrigerator_dummy + Dining_table_dummy + Dedicated_workspace_dummy + Microwave_dummy + Dishes_and_silverware_dummy + Pets_allowed_dummy + property_type_Entire home/apt + property_type_Entire serviced apartment + property_type_Private room in  
...
# Calculate the average RMSE
RMSE_OLS_2 <- mean(unlist(results))
