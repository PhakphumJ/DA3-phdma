...
kable(RMSE_Holdout, digits = 2, align = "c") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "bordered"))
...
hold_out_Mar %>%
  ...
  theme(plot.title = element_text(hjust = 0.5),  # Center the title
        ...
        axis.title = element_text(size = 12, face = "bold"))  # Adjust axis title size and make it bold
...
# OLS
RMSE_OLS_small <- RMSE(hold_out_Mar %>% filter(accommodates <= 3) %>% select(price), hold_out_Mar %>% filter(accommodates <= 3) %>% select(price_pred_ols))

RMSE_OLS_large <- RMSE(hold_out_Mar %>% filter(accommodates > 3) %>% select(price), hold_out_Mar %>% filter(accommodates > 3) %>% select(price_pred_ols))

# RF
RMSE_RF_small <- RMSE(hold_out_Mar %>% filter(accommodates <= 3) %>% select(price), hold_out_Mar %>% filter(accommodates <= 3) %>% select(price_pred_RF))

RMSE_RF_large <- RMSE(hold_out_Mar %>% filter(accommodates > 3) %>% select(price), hold_out_Mar %>% filter(accommodates > 3) %>% select(price_pred_RF))

# Bagging
RMSE_bag_small <- RMSE(hold_out_Mar %>% filter(accommodates <= 3) %>% select(price), hold_out_Mar %>% filter(accommodates <= 3) %>% select(price_pred_bag))

RMSE_bag_large <- RMSE(hold_out_Mar %>% filter(accommodates > 3) %>% select(price), hold_out_Mar %>% filter(accommodates > 3) %>% select(price_pred_bag))
...
```{r}
## Compile the results (RMSE, RMSE/Mean(Price)) in a table.

RMSE_Holdout_small <- data.frame(RMSE_OLS_Holdout, RMSE_RF_Holdout, RMSE_bagging_optm_Holdout) %>%
  pivot_longer(everything(), names_to = "model", values_to = "RMSE") %>%
  mutate(model = c("OLS", "RF", "Bagging"))
