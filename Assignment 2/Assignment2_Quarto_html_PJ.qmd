...
1. Model 1: number of people that can be accommodated (linear)
2. Model 2: Model 1 + characteristics of the living space (e.g. number of bathrooms, number of beds, room types) except amenities 
3. Model 3: Model 2 + squared terms for numerical variables.
4. Model 4: Model 3 + variables related to reviews (only those with correaltion less than 0.7 with each other)
5. Model 5: Model 4 + information about rail transportation
6. Model 6: Model 5 + information about the neighborhood
7. Model 7: Model 6 + information about the host 
8. Model 8: All variables


Model 1: *price* = *f*(*accommodates*)

Model 2: *price* = model 1 + *bedrooms*, *beds*, *property_type*, *room_type*, *num_baths*, *num_shared_baths*, *is_half_bath*, *num_private_baths*, *bedrooms_imputed*

Model 3: *price* = model 2 + $accommodates^2$, $bedrooms^2$, $beds^2$, $num_baths^2$, $num_shared_baths^2$, $num_private_baths^2$

Model 4: *price* = model 3 + *review_scores_rating*, *review_scores_cleanliness*, *review_scores_communication*, *review_scores_location*, *review_scores_rating_imputed*, *time_since_last_review*, *number_of_reviews*

Model 5: *price* = model 4 + *near_MRT*, *near_BTS*, *near_ARL*

Model 6: *price* = model 5 + *neighborhood*

Model 7: *price* = model 6 + *host_is_superhost*, *host_response_time*, *host_response_rate*, *host_acceptance_rate*, *host_total_listings_count*, *host_identity_verified*
...
for (i in 1:5) {
  ...
  # Estimate the model
  model_OLS_1 <- lm(price ~ accommodates, data = train)
...
for (i in 1:5) {
  ...
  # Estimate the model
  model_OLS_3 <- update(model_OLS_2, . ~ . + accommodates^2 + bedrooms^2 + beds^2 + num_baths^2 + num_shared_baths^2 +  num_private_baths^2 , data = train)
...
for (i in 1:5) {
  ...
  # Estimate the model
  model_OLS_5 <- update(model_OLS_4, . ~ . + near_MRT + near_BTS + near_ARL, data = train)
...
results <- data.frame(
  Model = c("OLS_1", "OLS_2", "OLS_3", "OLS_4", "OLS_5", "OLS_6", "OLS_7", "OLS_7"),
...
Model 2 and Model 3 have the lowest RMSE. I will choose Model 2 as the final model because it has the lowest number of variables.
