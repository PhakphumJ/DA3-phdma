...
**Model 3:** **Model 2** + *(male, ind02)*\
&nbsp;\
meaning that I accounted for fixed characteristics of each industry & gender.\

**Model 4:** **Model 3** + *(Minwage, male)*\
&nbsp; \
...
for (i in 1:5) {
  ...
  model_1 <- lm(earnhr ~ age + marr_abs + wid_div + nevmarr + noncitiz + natura + than7nodip + HS_GED + Col_ND + asscd + Bach_more, weights = weight) 
...
for (i in 1:5) {
  ...
  # Train your OLS model
  model_3 <- update(model_2, .~. + male, ind02) 
  # Assuming 'target_variable' is the variable you're trying to predict, 
  # and you want to use all other variables as predictors
...
for (i in 1:5) {
  ...
  # Train your OLS model
  model_4 <- update(model_3, .~. + Minwage, male) 
  # Assuming 'target_variable' is the variable you're trying to predict, 
  # and you want to use all other variables as predictors
