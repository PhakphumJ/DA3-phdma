...
*ethnic* is also discarded since it mainly describes the ethnicity of Hispanic workers ,which I think might not be very useful. It also contains many categories. If included, we would lose quite degree of freedom.
...
WorkingData = MiscWorkerData %>% select(-c(...1, hhid, intmonth, earnwke, race, ethnic, sex, marital, chldpres, prcitshp, state, occ2012, class, unionmme, unioncov, lfsr94, grade92))
```
...
correlations = cor(WorkingData[,-c(1,2,6)])

# select only correlation y and x
target_correlations = correlations["earnhr", ]

# put it in a nice table.
target_correlations <- data.frame(
  Feature = colnames(correlations),
  Correlation = correlations["earnhr", ]
)
```

```{r}
# Print the correlations
print(target_correlations[2])
```
