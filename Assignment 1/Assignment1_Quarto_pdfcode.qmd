...
Link to code on Github: <https://github.com/PhakphumJ/DA3-phdma/tree/main/Assignment%201> <span style="color:grey;">(.qmd file may be preferable to the R-script due to better readability.)</span>
...
## Performance
The performance metrics of the estimated models are present in @tbl-performance and @fig-RMSE-plot.
...
ggplot(df_plot, aes(x = Model, y = Value, color = Metric, group = Metric)) +
  ...
  theme_minimal() + theme(axis.text = element_text(size = 10), plot.margin = margin(1, 1, 1, 1, "cm"))  
